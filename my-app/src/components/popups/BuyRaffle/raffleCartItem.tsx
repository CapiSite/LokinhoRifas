...
const RaffleCartItem = ({
  ...
  const { setRaffleSelected, setShowNumberPicker } =
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (maxQuantity < Number(e.target.value))
      return setDefaultValue(maxQuantity.toString());
    if (Number(e.target.value) < 0) return setDefaultValue("0");
    setDefaultValue(e.target.value);
  };

  const newValue = value.toString().includes(".")
    ? `${value.toString().split(".")[0]},${value.toString().split(".")[1][0]}${
    ...
    <div className="cartItem">
      <div className="raffleBanner desktop">
        <div className="glowGroup">
          <div
            className={`glow-1 ${bundleValue > 1000 ? "Gold" : "Silver"}`}
          ></div>
          <div
            className={`glow-2 ${bundleValue > 1000 ? "Gold" : "Silver"}`}
          ></div>
        </div>
        <Image
          className="skin"
          ...
          src={imgSrc}
          alt="Skin principal"
          onError={() => setImgSrc(defaultGunPic)}
        />
      ...
      <div className="raffleMetaData">
        ...
        <p className="currentQuantity">
          Números restantes: {maxQuantity} - {quantity}
        </p>
      </div>
    </div>
  );
